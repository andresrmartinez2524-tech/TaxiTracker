    ...
    <div style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh' }}>
      ...
      <header style={{ borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-surface)', padding: '0 1.5rem' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            ...
            <span style={{ fontSize: '0.9375rem', color: 'var(--text-muted)' }}>Administrador</span>
          </div>
          <button
            onClick={() => {
      ...
      <div className="container">
        ...
        <div style={{ marginBottom: '1.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ fontSize: '1.375rem', fontWeight: '600', color: 'var(--text-main)', letterSpacing: '-0.025em', marginBottom: '0.25rem' }}>Panel de Administrador</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Visión global de todos los reportes</p>
          </div>
          
          <button
            onClick={() => {
              import('../utils/exportExcel').then(module => {
                module.exportToExcel(reports);
              });
            }}
